...
scr_arg6 <- function(nval = 200) {
  ...
  legend("bottomleft", col = c("orange", "blue"),
    legend = c("Species A", "Species B"), lty = 1, cex = 1, lwd = 3, bty = "n")
  text(-0.02, 0.98, "a")
  ...
  mtext(side = 1, line = 1, text = "Interaction strength", outer = TRUE, cex = 1)
  text(-0.02, 0.18, "b")
...
model <- function(Time, State, Pars) {
	with(as.list(c(State,Pars)), {
		...
		dp12dt <- (f1*p1 + fp1*p12)*sp1*p2 + (f2*p2 + fp2*p12)*sp2*p1 -
			(ep1 + ep2)*p12
...
runeq <- function(pars) {
	# Initial conditions
	pstart <- c(p1 = 0.5, p2 = 0.5, p12 = 0.0)
