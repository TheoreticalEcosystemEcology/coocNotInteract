...
scr_arg1 <- function() {
  ...
  prob1 <- logistic2(seq_env - 2.5, yzer = .5, ypos = 0.01, yneg = .99,
    lambda = 2)
  prob2 <- logistic2(seq_env - 2.5, yzer = .5, yneg = 0.01, ypos = .99,
    lambda = 2)
  # Environemnt
  ...
  mat_lay <- cbind(
    rbind(rep(1, 4), rep(2:3, each = 2), rep(4:5, each = 2), 6)
  )

  png("output/fig1.png", res = 300, width = 6, height = 5, units = "in")
    layout(mat_lay, heights = c(.5, 1,1, .15), widths = rep(1, 6))
    ...
    text(c(.8,4.2), c(.8,.8), labels = c(expression(P(X[A]*"|"*E)),
    ...
    par(mar = c(2, 7, 2, 1))
      ...
      axis(1, lwd = 0, lwd.ticks = .5)
      axis(2, lwd = 0, lwd.ticks = .5)
      title(ylab = expression(P(X[A]*","*X[B]*"|"*E)*P(E)), cex.lab = 1.5,
        line = 4)
      box2(1:2, lwd = 1.2)
      mtext(expression(P(X[A]*","*X[B])), 3, cex = .8, at = 2.5, adj = 1)
      mtext(paste0(" = ", format(ls_sprob[[i]], digits = 3)), 3, cex = .76,
        at = 2.5, line = .4, adj = 0)
...
# mat_lay <- cbind(
#   c(1, 2, 6, 0),
#   rbind(rep(1, 6), rep(2:3, each = 3), rep(4:5, each = 3), 7)
# )