...
scr_arg1 <- function() {
  ...
  text(c(0.8, 4.2), c(0.8, 0.8), labels = c(expression(P(X[A] * "|" * E)),
    expression(P(X[B] * "|" * E))), col = pal[c(3, 2)], cex = 1.4)
...
scr_arg1_2 <- function() {
  denv <- 0.01
  seq_env <- seq(0, 5, denv)
  prob1 <- logistic2(seq_env - 2.5, yzer = 0.5, ypos = 0.01, yneg = 0.99, lambda = 2)
  prob2 <- logistic2(seq_env - 2.5, yzer = 0.5, yneg = 0.01, ypos = 0.99, lambda = 2)

  env0 <- env1 <- env2 <- env3 <- rep(0, length(seq_env))
  env1[seq_env >= 0 & seq_env <= 5] <- 0.2
  env2[seq_env >= 1.5 & seq_env <= 3.5] <- .5
  # env3[seq_env >= 0 & seq_env <= 1] <- 0.5
  env3[seq_env >= 0 & seq_env <= 2.5] <- seq(.8, 0, length.out = .5*length(env0))

  ls_envi <- list(env1, env2, env3)
  ls_prob <- list(prob1 * prob2 * env1 * denv, prob1 * prob2 * env2 * denv, prob1 *
    prob2 * env3 * denv)
  ls_sprob <- lapply(ls_prob, sum)

  mlet <- function(let = "a", line = -1) mtext(3, text = let, adj = 0.02, line = line)


  # output dir
  output_dir()

  mat_lay <- cbind(c(1, 2, 10, 11, 0), rbind(rep(1, 6), rep(2:3, each = 3), matrix(rep(4:9,
    each = 2), byrow = TRUE, nrow = 2), 12))


  png("output/fig1_2.png", res = 600, width = 6, height = 7, units = "in")
